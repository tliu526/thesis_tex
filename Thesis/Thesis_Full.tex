...
\subsection{Input/Output Files and Data Analysis}

We have a standardized format for storing and reading in grid configurations, which allows us to easily stop and restore CA simulations as well as save and transfer them across different machines. A representative input grid file can be found in Appendix TODO. The visual representation of our grids are built by creating graph descriptions (.dot file extension) for \textbf{neato}, a graph layout program part of the Graphviz drawing package~\cite{el01}. Images of our neato grid representations can be found in Section \ref{sec:GridGen}.
